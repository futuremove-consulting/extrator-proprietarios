"""Módulo persister para Fisgar - responsável pela persistência de dados."""
...
def persistir_proprietario(dados_extraidos: Dict[str, Any], 
    ...
    markdown = _gerar_markdown_proprietario_fisgar(registro_completo)
...
def _gerar_markdown_proprietario_fisgar(registro: Dict[str, Any]) -> str:
    """Gera representação Markdown do proprietário do Fisgar."""
    ...
    md = f"""# Proprietário: {manifest.get('name_raw', 'N/A')} (Fisgar)
...
"""
    ...
    for i, tel in enumerate(telefones, 1):
        ...
        if tel.get('observacao'):
            md += f" - {tel.get('observacao')}"
        md += "\n"
...
def atualizar_proprietario_existente(dados_atualizados: Dict[str, Any],
    ...
    # Recriar markdown
    markdown = _gerar_markdown_proprietario_fisgar(dados_existentes)
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(markdown)
